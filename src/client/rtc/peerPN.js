export function createPeerPN({polite, signaling, log}) {
  ...
  async function handleSignal({description, candidate}) {
    try {
      if (description) {
        ...
        const offerCollistion = description.type === 'offer' && !readyForOffer;
        if (offerCollistion) {
          if (!polite) {
            // impolite: 충돌 시 상대 offer 무시
            log('[PN] offer collistion: impolite -> ignore remote offer');
            return;
          }

          // polite 내가 하던 것 되돌리고(rollback) 상대 offer를 수락
          log('[PN] offer collition: polite -> rollback & accept remote offer');
        ...
        if (description.type === 'offer') {
          ...
          if (description.type === 'answer') {
            ...
            // setRemoteDescription 바로 위에서 수행되었음
            queueMicrotask(() => {
        ...
        try {
          ...
          log('[PN] addIceCandidate error(무시 가능) : ' + error?.message);
        }
      }
    } catch (error) {
  ...
  function send(text) {
    ...
    else log(`[dc] not opne (state=${dc?.readyState})`);
  }
