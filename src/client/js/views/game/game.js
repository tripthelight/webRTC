...
signaling.onMessage(async msg => {
  if (msg.type === 'joined') {
    ...
    // 방에 이미 2명인 상태로 합류했다면(내가 두번째) - 바로 킥
    if (msg.count === 2) {
    ...
    // 상대가 떠났다면: 내 쪽 RTCPeerConnection 을 정리하고 "새 연결 받을 준비" 상태로 전환
    try {
      peer?.close();
    } catch {}
    peer = null;
    // 나만 남았으므로 다음 상대가 들어오면 내가 inpolite가 되어 채널을 먼저 만들어야 함
    polite = false;
    // 새 피어를 미리 만들어 둔다(상대가 돌아오면 즉시 킥 가능)
    if (polite !== null) {
      peer = createPeerPN({polite, signaling, log});
      // 여기서는 아직 ensureNegotiationKick를 호출하지 않음
      // 상대가 돌아와 peer-join을 발생 시킬 때 즉시 킼
    }
  }
});
